...
do
	./bin/psiblast -query $TMPDIR/$id.seq -db ./db/swissprot -num_iterations 3 -num_threads  10 -out $TMPDIR/$id.out -out_ascii_pssm $TMPDIR/$id.pssm 
...
cat $TMPDIR/idlist|while read line;do
	...
	/opt/Dispredict3.0/.venv/bin/python3 Disnet.py ./tmp$number/$line.ttscore > ./tmp$number/$line.ttpreds
