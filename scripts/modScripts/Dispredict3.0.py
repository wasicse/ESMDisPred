...
def dispredict(fasta_filepath,output_path):
    ...
    if flagfeatures:      
        ...
        bashCommand="/opt/Dispredict3.0/.venv/bin/python "+parent_path+"/tools/fldpnn/run_flDPnn.py "+fasta_filepath
        output = subprocess.check_output(["bash","-c", bashCommand])
        print(output.decode('utf-8')) 
        ...
        for record in SeqIO.parse(fasta_filepath, "fasta"):
            ...
            with open(output_path+"predictions/"+pid+"_disPred.caid", "ab") as f:
                f.write((">"+pid+"\n").encode())
                fmt = '%s', '%s', '%1.3f', '%s'
